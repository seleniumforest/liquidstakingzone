CREATE OR REPLACE VIEW Stride.depositors_volume AS WITH ranges as (
    ...
    prices as (
        SELECT dt,
            AVG(price) as price
        FROM (
                SELECT toStartOfDay(toDateTime64(date / 1000, 3, 'Etc/UTC')) as dt,
                    *
                FROM Stride.price_history ph
                WHERE vsCurrency = 'usd'
                    and coin = (
                        SELECT TOP 1 coingeckoId
                        FROM Stride.zones_info zi2
...
SELECT range,
    COUNT(am) as count
FROM (
        SELECT toStartOfDay(toDateTime64(mmls.date, 3, 'Etc/UTC')) as dt,
        ...
        WHERE mmls.zone = { zone_name: String }
            AND am > range.1
            AND am < range.2
            AND txcode = 0
        ORDER BY dt
